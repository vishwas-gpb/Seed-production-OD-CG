...
function cascadeFill(p){
  const st=el(p+"state").value, agSel=el(p+"agency").value, dSel=el(p+"district").value, bSel=el(p+"block").value, gSel=el(p+"gp").value, vSel=el(p+"village").value;
  const inS=FARMERS.filter(f=>!st||f.state===st); fillSelectKeep(p+"agency",uniqueSorted(inS.map(f=>f.agency)),agSel);
  const ag=el(p+"agency").value; const inA=inS.filter(f=>!ag||f.agency===ag); fillSelectKeep(p+"district",uniqueSorted(inA.map(f=>f.district)),dSel);
  const d=el(p+"district").value; const inD=inA.filter(f=>!d||f.district===d); fillSelectKeep(p+"block",uniqueSorted(inD.map(f=>f.block)),bSel);
  const b=el(p+"block").value; const inB=inD.filter(f=>!b||f.block===b); fillSelectKeep(p+"gp",uniqueSorted(inB.map(f=>f.gp)),gSel);
  const g=el(p+"gp").value; const inG=inB.filter(f=>!g||f.gp===g); fillSelectKeep(p+"village",uniqueSorted(inG.map(f=>f.village)),vSel);
}
...
function kpiCard(v,l,extra){ return "<div class='kpi'><div class='kpi-v'>"+v+(extra||"")+"</div><div class='kpi-l'>"+l+"</div></div>"; }
function renderKPI(s){
  ...
  el("kpi").innerHTML=kpiCard(s.total||0,"total visits")+kpiCard(thisW,"visits this week",trendArrow(thisW,lastW))+kpiCard(pct+"%","farmers covered")+kpiCard(pending,"not yet visited")+kpiCard(s.avgRevisitDays||0,"avg days / revisit");
}
let MAP=null, MAP_LAYER=null;
function renderMap(points){
  const div=el("map"); if(!div||typeof L==="undefined") return;
  const pts=(points||[]).map(p=>({la:parseFloat(p.lat),lo:parseFloat(p.lon),p:p})).filter(o=>!isNaN(o.la)&&!isNaN(o.lo));
  ...
  if(!pts.length){ setTimeout(()=>MAP.invalidateSize(),120); return; }
  MAP_LAYER=L.layerGroup(); const bounds=[];
  pts.forEach(o=>{ bounds.push([o.la,o.lo]);
    L.circleMarker([o.la,o.lo],{radius:6,color:"#fff",weight:1,fillColor:condColor(o.p.condition),fillOpacity:.9})
      .bindPopup("<b>"+esc(o.p.farmer_name||"")+"</b><br>"+esc(o.p.village||"")+"<br>"+esc(o.p.crop||"")+" - "+esc(o.p.condition||"")+"<br>"+esc(o.p.visit_date||"")).addTo(MAP_LAYER); });
  MAP_LAYER.addTo(MAP); try{ MAP.fitBounds(bounds,{padding:[24,24],maxZoom:13}); }catch(e){}
...
async function loadTeam(){
  if(!navigator.onLine){ el("kpi").innerHTML="<span class='muted'>Offline - connect to load the team dashboard.</span>"; return; }
  if(ENDPOINT.indexOf("PASTE_YOUR")===0){ el("kpi").innerHTML="<span class='muted'>Set ENDPOINT first.</span>"; return; }
  el("kpi").innerHTML="<span class='muted'>Loading...</span>";
  try{ const s=await api({action:"stats"});
    TEAM_VISITED=new Set(s.visitedIds||[]);
    renderKPI(s); renderMap(s.points||[]); renderCharts(s); renderTraffic(s);
    ...
    renderNotVisited();
  }catch(e){ el("kpi").innerHTML="<span class='muted'>Could not load team dashboard.</span>"; } }
...
async function loadGallery(){
  ...
  if(GALLERY_SOURCE==="device"){
    ...
    GALLERY_ITEMS=items; renderBands(); renderGallery();
  } else {
    ...
    try{ const out=await api({action:"photos"}); GALLERY_ITEMS=out.items||[]; renderBands(); renderGallery(); }
...
function inTime(it){ if(GALLERY_TIME==="all") return true; const d=String(it.visit_date||"").slice(0,10); if(!d) return false; const now=new Date(), today=now.toISOString().slice(0,10);
  if(GALLERY_TIME==="today") return d===today; const dt=new Date(d); if(isNaN(dt)) return false;
  if(GALLERY_TIME==="week"){ const diff=(now-dt)/86400000; return diff>=0&&diff<7; } if(GALLERY_TIME==="month") return d.slice(0,7)===today.slice(0,7); return true; }
