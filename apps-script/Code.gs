...
var V_HEADERS = ["visit_id","visit_date","visit_time","staff_id","staff_name","farmer_id","farmer_name",
  "district","block","gp","village","crop","seed_variety","seed_class",
  "crop_stage","condition","purpose","lat","lon","notes",
  "photo1","photo2","photo3","logged_at","device_id","received_at"];
var U_HEADERS = ["staff_id","name","phone","salt","pin_hash","status","registered_at","approved_at"];
...
function register(b) {
  ...
  s.appendRow([b.staff_id, b.name || "", b.phone || "", salt, hashPin(b.pin, salt), "pending", new Date(), ""]);
...
function login(b) {
  ...
  var salt = u.values[3], hash = u.values[4], state = String(u.values[5]);
  ...
  return json({ status: "ok", name: u.values[1], salt: salt, pin_hash: hash });
}
