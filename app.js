...
function api(payload){ return fetch(ENDPOINT,{method:"POST",body:JSON.stringify(Object.assign({token:TOKEN},payload))}).then(r=>r.json()); }
async function apiRead(payload,tries){ tries=tries||3; let err;
  for(let i=0;i<tries;i++){ try{ const r=await fetch(ENDPOINT,{method:"POST",body:JSON.stringify(Object.assign({token:TOKEN},payload))}); if(!r.ok) throw new Error("HTTP "+r.status); return await r.json(); }
...
function applyRole(role){
  const viewer=(role==="viewer"), reviewer=(role==="reviewer"); const canLog=!(viewer||reviewer);
  el("tab-new").style.display=canLog?"":"none"; el("tab-rec").style.display=canLog?"":"none";
  el("tab-farmers").style.display=(viewer||reviewer)?"":"none"; el("tab-review").style.display=reviewer?"":"none"; el("tab-tasks").style.display=canLog?"":"none";
  if(viewer){ GALLERY_SOURCE="team"; const gs=el("gal_source"); if(gs) gs.value="team"; el("who").textContent+=" (management)"; showView("farmers"); }
  if(reviewer){ GALLERY_SOURCE="team"; const gs=el("gal_source"); if(gs) gs.value="team"; el("who").textContent+=" (reviewer)"; showView("review"); }
}
...
function showView(v){ ["new","rec","tasks","review","farmers","photos","dash"].forEach(k=>{ const s=el("view-"+k); if(s) s.classList.toggle("hidden",k!==v); const t=el("tab-"+k); if(t) t.classList.toggle("active",k===v); }); if(v==="rec") render(); if(v==="dash") dashboard(); if(v==="photos") loadGallery(); if(v==="farmers"){ renderFarmerDir(); loadVisits(false); } if(v==="review"){ loadVisits(false).then(renderReviewList); } if(v==="tasks"){ loadVisits(false).then(renderTasks); } }
...
async function saveVisit(){
  ...
  if((await idbAll()).some(x=>x.farmer_id===f.farmer_id && x.visit_date===today && (x.seed_variety||"")===(f.seed_variety||"") && (x.crop||"")===(f.crop||""))){ toast("This farmer's crop/variety is already recorded today."); return; }
...
function renderTeam(s){
  ...
  renderKPI(s); renderMap(LAST_TEAM_POINTS); renderCharts(s); renderTraffic(s); renderAttention(s.attention||[]);
...
async function loadVisits(force){
  ...
  if(!force && VISITS_LOADED) return;
  if(!navigator.onLine){ const c=cacheGet("visits"); if(c){ ALL_VISITS=c; VISITS_LOADED=true; } return; }
  try{ const out=await apiRead({action:"visits"},3); ALL_VISITS=out.items||[]; cacheSet("visits",ALL_VISITS); VISITS_LOADED=true; }
  catch(e){ const c=cacheGet("visits"); if(c){ ALL_VISITS=c; VISITS_LOADED=true; } }
}
function reviewFlag(v){ const r=v.review; if(!r) return "<span class='badge'>new</span>";
  if(r.next_visit_date && r.next_visit_date<localDate() && String(r.actions_done).toLowerCase()!=="yes") return "<span class='badge' style='background:#fdecea;color:#c62828'>attention</span>";
  return "<span class='badge' style='background:#e8f5e9;color:#2e7d32'>reviewed</span>"; }
function renderReviewList(){
  const box=el("review_list"); if(!box) return;
  if(!ALL_VISITS.length){ box.innerHTML="<div class='muted' style='padding:10px'>No visits yet, or tap Reload.</div>"; return; }
  box.innerHTML=ALL_VISITS.slice(0,150).map(v=>"<div class='res' data-v='"+esc(v.visit_id)+"'>"+esc(v.farmer_name||"?")+" - "+esc(v.village||"")+" - "+esc(v.visit_date||"")+" - "+esc(v.condition||"")+" "+reviewFlag(v)+"</div>").join("");
  box.querySelectorAll(".res[data-v]").forEach(x=>x.addEventListener("click",()=>openReview(x.dataset.v)));
}
function openReview(vid){
  const v=ALL_VISITS.find(x=>x.visit_id===vid); if(!v) return; REVIEW_VID=vid; const r=v.review||{};
  el("rv_head").innerHTML="<b>"+esc(v.farmer_name||"")+"</b> - "+esc(v.village||"")+"<br>"+esc(v.visit_date||"")+" - stage "+esc(v.crop_stage||"-")+" - condition <span style='color:"+condColor(v.condition)+"'>"+esc(v.condition||"-")+"</span>"+(v.immediate_actions?"<br><span class='muted'>Field action: "+esc(v.immediate_actions)+"</span>":"");
  el("rv_photos").innerHTML=(v.photos||[]).map(p=>"<img src='"+p+"' style='width:84px;height:84px;object-fit:cover;border-radius:6px;margin:2px'>").join("");
  el("rv_comment").value=r.comment||""; el("rv_condition").value=r.condition_review||""; el("rv_actions").value=r.actions||""; el("rv_next").value=r.next_visit_date||""; el("rv_done").checked=String(r.actions_done).toLowerCase()==="yes";
  el("review_form").classList.remove("hidden"); el("review_form").scrollIntoView({behavior:"smooth",block:"nearest"});
}
function closeReview(){ el("review_form").classList.add("hidden"); REVIEW_VID=null; }
async function saveReview(){
  if(!REVIEW_VID) return; const v=ALL_VISITS.find(x=>x.visit_id===REVIEW_VID)||{};
  const rev={ visit_id:REVIEW_VID, farmer_id:v.farmer_id||"", farmer_name:v.farmer_name||"", village:v.village||"", reviewer:(SESSION&&(SESSION.name||SESSION.staff_id))||"",
    comment:el("rv_comment").value.trim(), condition_review:el("rv_condition").value, actions:el("rv_actions").value.trim(), next_visit_date:el("rv_next").value, actions_done:el("rv_done").checked?"yes":"no" };
  if(!rev.actions && !rev.comment){ toast("Add a comment or an action"); return; }
  toast("Saving review...");
...
function renderTimeline(farmerId){
  ...
  box.innerHTML="<div class='muted' style='margin:12px 0 4px'>Progress timeline (oldest &rarr; newest)</div>"+visits.map(v=>{
    ...
    return "<div style='border-left:3px solid "+col+";padding:7px 10px;margin:8px 0;background:#fafbfa;border-radius:6px'>"+
      "<b>"+esc(v.visit_date||"")+"</b> &middot; "+esc(v.crop_stage||"-")+" &middot; <span style='color:"+col+"'>"+esc(v.condition||"-")+"</span>"+
      (ph?"<div style='margin-top:5px'>"+ph+"</div>":"")+
      (v.immediate_actions?"<div style='font-size:13px;margin-top:4px'><b>Field action:</b> "+esc(v.immediate_actions)+"</div>":"")+
      (r&&r.actions?"<div style='font-size:13px;margin-top:2px'><b>Reviewer:</b> "+esc(r.actions)+(r.next_visit_date?" <span class='muted'>(next: "+esc(r.next_visit_date)+")</span>":"")+"</div>":"")+
    "</div>";
  }).join("");
}
function myTasks(){ if(!SESSION) return []; return ALL_VISITS.filter(v=>String(v.staff_id)===String(SESSION.staff_id) && v.review && v.review.actions && String(v.review.actions_done).toLowerCase()!=="yes"); }
function updateTaskBadge(){ const b=el("task_badge"); if(!b) return; const n=myTasks().length; b.textContent=n; b.style.display=n?"":"none"; }
function renderTasks(){
  const box=el("task_list"); if(!box) return; const tasks=myTasks();
  updateTaskBadge();
  if(!tasks.length){ box.innerHTML="<div class='muted' style='padding:10px'>No pending actions assigned to you.</div>"; return; }
  tasks.sort((a,b)=>String(a.review.next_visit_date||"9999").localeCompare(String(b.review.next_visit_date||"9999")));
  box.innerHTML=tasks.map(v=>{ const r=v.review, overdue=r.next_visit_date && r.next_visit_date<localDate();
    return "<div class='res' style='border-left:3px solid "+(overdue?"#c62828":"#2e7d32")+"'><b>"+esc(v.farmer_name||"")+"</b> - "+esc(v.village||"")+(r.next_visit_date?" - by "+esc(r.next_visit_date)+(overdue?" <span style='color:#c62828'>(overdue)</span>":""):"")+"<br>"+esc(r.actions||"")+(r.comment?"<br><span class='muted'>"+esc(r.comment)+"</span>":"")+"</div>"; }).join("");
}
function renderAttention(list){
  const box=el("attention"); if(!box) return; list=list||[];
  if(!list.length){ box.innerHTML="<span class='muted'>Nothing overdue right now.</span>"; return; }
  list.sort((a,b)=>b.days_overdue-a.days_overdue);
  box.innerHTML=list.slice(0,60).map(a=>"<div class='res' style='border-left:3px solid #c62828'><b>"+esc(a.farmer_name||"")+"</b> - "+esc(a.village||"")+" - due "+esc(a.next_visit_date||"")+" <span style='color:#c62828'>("+a.days_overdue+"d overdue)</span>"+(a.actions?"<br><span class='muted'>"+esc(a.actions)+"</span>":"")+"</div>").join("");
}
/* ---------- boot ---------- */
window.addEventListener("load",()=>{ const d=el("view-dash"); if(d && !d.classList.contains("hidden")){ try{ farmersDashboard(); renderMap(LAST_TEAM_POINTS); }catch(e){} } });
...
function initApp(){ fillSelectPH("crop_stage",STAGES,"- select stage -"); fillSelectPH("condition",CONDITION,"- select condition -"); fillSelectPH("purpose",PURPOSES,"- select purpose -"); setNet(); updateCounts(); loadFarmers(); setTimeout(()=>syncNow(false),1500); setTimeout(()=>{ if(el("tab-tasks")&&el("tab-tasks").style.display!=="none") loadVisits(false).then(updateTaskBadge); },1900); }
(function boot(){
