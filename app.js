...
function applyRole(role){ const viewer=(role==="viewer"); el("tab-new").style.display=viewer?"none":""; el("tab-rec").style.display=viewer?"none":""; if(viewer){ GALLERY_SOURCE="team"; const gs=el("gal_source"); if(gs) gs.value="team"; el("who").textContent+=" (management)"; showView("dash"); } }
...
function showView(v){ ["new","rec","photos","dash"].forEach(k=>{ const s=el("view-"+k); if(s) s.classList.toggle("hidden",k!==v); const t=el("tab-"+k); if(t) t.classList.toggle("active",k===v); }); if(v==="rec") render(); if(v==="dash") dashboard(); if(v==="photos") loadGallery(); }
...
function initFilters(){ fillSelectKeep("f_state",uniqueSorted(FARMERS.map(f=>f.state)),""); ["f_agency","f_district","f_block","f_gp","f_village"].forEach(id=>fillSelect(id,[],true)); onFilter(); }
function currentPool(){ const st=el("f_state").value,ag=el("f_agency").value,d=el("f_district").value,b=el("f_block").value,g=el("f_gp").value,v=el("f_village").value;
  return FARMERS.filter(f=>(!st||f.state===st)&&(!ag||f.agency===ag)&&(!d||f.district===d)&&(!b||f.block===b)&&(!g||f.gp===g)&&(!v||f.village===v)); }
function onFilter(){
  const st=el("f_state").value, agSel=el("f_agency").value, dSel=el("f_district").value, bSel=el("f_block").value, gSel=el("f_gp").value, vSel=el("f_village").value;
  const inS=FARMERS.filter(f=>!st||f.state===st); fillSelectKeep("f_agency",uniqueSorted(inS.map(f=>f.agency)),agSel);
  const ag=el("f_agency").value; const inA=inS.filter(f=>!ag||f.agency===ag); fillSelectKeep("f_district",uniqueSorted(inA.map(f=>f.district)),dSel);
  const d=el("f_district").value; const inD=inA.filter(f=>!d||f.district===d); fillSelectKeep("f_block",uniqueSorted(inD.map(f=>f.block)),bSel);
  const b=el("f_block").value; const inB=inD.filter(f=>!b||f.block===b); fillSelectKeep("f_gp",uniqueSorted(inB.map(f=>f.gp)),gSel);
  const g=el("f_gp").value; const inG=inB.filter(f=>!g||f.gp===g); fillSelectKeep("f_village",uniqueSorted(inG.map(f=>f.village)),vSel);
  selectedFarmer=null; el("farmercard").classList.add("hidden"); onFarmerSearch();
}
...
function showFarmerCard(){ const f=selectedFarmer; if(!f){ el("farmercard").classList.add("hidden"); return; }
  ...
  el("farmercard").classList.remove("hidden");
  el("farmercard").innerHTML="<b>"+esc(f.farmer_name||"")+"</b> ("+esc(f.farmer_id||"")+")<br>"+(f.father_name?("F/H: "+esc(f.father_name)+"<br>"):"")+
    esc([f.village,f.gp,f.block,f.district,f.state].filter(Boolean).join(", "))+(f.agency?" - "+esc(f.agency):"")+"<br>Crop: "+esc(f.crop||"-")+" - Variety: "+esc(f.seed_variety||"-")+" - Class: "+esc(f.seed_class||"-")+"<br>Sown: "+esc(f.date_sowing||"-")+" - Area: "+esc(f.area_ha||"-")+" ha"+extra; }
...
async function saveVisit(){
  ...
  const v={ synced:false, device_id:deviceId(), visit_id:"V-"+now.getTime().toString(36)+"-"+Math.random().toString(36).slice(2,5),
    ...
    visit_date:now.toISOString().slice(0,10), visit_time:now.toTimeString().slice(0,8), lat:window._lat||"", lon:window._lon||"",
    crop_stage:el("crop_stage").value, condition:el("condition").value, purpose:el("purpose").value,
    immediate_actions:el("immediate_actions").value, notes:el("notes").value, logged_at:now.toISOString(), photos:photos.filter(Boolean) };
...
async function syncNow(manual){
  ...
  try{ for(const v of pending){ try{ const out=await api({action:"visit",visit:v}); if(out&&out.status==="ok"){ v.synced=true; await idbPut(v); ok++; } }catch(e){ break; } }
...
function farmersDashboard(){
  ...
  el("fm_stats").innerHTML="<span class='stat'><b>"+FARMERS.length+"</b>registered farmers</span><span class='stat'><b>"+totalArea+"</b>total area (ha)</span><span class='stat'><b>"+crops+"</b>crops</span><span class='stat'><b>"+villages+"</b>villages</span>";
  bars("fm_crop_area", sumBy(FARMERS,"crop","area_ha"));
  bars("fm_state_area", sumBy(FARMERS,"state","area_ha"));
  bars("fm_agency_area", sumBy(FARMERS,"agency","area_ha"));
...
async function loadTeam(){
  ...
  try{ const s=await api({action:"stats"});
    ...
    const reg=FARMERS.length, vis=s.farmers||0, pct=reg?Math.round(vis/reg*100):0;
    el("rc_stats").innerHTML="<span class='stat'><b>"+pct+"%</b>farmers covered</span><span class='stat'><b>"+vis+"</b>of "+(reg||"?")+" visited</span><span class='stat'><b>"+(s.avgRevisitDays||0)+"</b>avg days between revisits</span>";
    ...
    TEAM_VISITED=new Set(s.visitedIds||[]); renderNotVisited();
...
let GALLERY_SOURCE="device", GALLERY_ITEMS=[], GALLERY_SORTED=[], GALLERY_SORT="new", LB_I=0;
async function loadGallery(){
  ...
  if(GALLERY_SOURCE==="device"){
    ...
    all.forEach(v=>{ (v.photos||[]).forEach(p=>items.push({url:p,farmer_name:v.farmer_name,village:v.village,crop:v.crop,seed_variety:v.seed_variety,area_ha:v.area_ha,lat:v.lat,lon:v.lon,visit_date:v.visit_date,crop_stage:v.crop_stage,condition:v.condition})); });
    GALLERY_ITEMS=items; renderGallery();
  } else {
    if(!navigator.onLine){ g.innerHTML="<span class='muted'>Offline - connect to load team photos.</span>"; return; }
    if(ENDPOINT.indexOf("PASTE_YOUR")===0){ g.innerHTML="<span class='muted'>Set ENDPOINT first.</span>"; return; }
    g.innerHTML="<span class='muted'>Loading photos...</span>";
    try{ const out=await api({action:"photos"}); GALLERY_ITEMS=out.items||[]; renderGallery(); }
    catch(e){ g.innerHTML="<span class='muted'>Could not load photos.</span>"; }
  }
}
function sortGallery(){ const s=el("gal_sort"); GALLERY_SORT=s?s.value:"new"; renderGallery(); }
function renderGallery(){
  let items=GALLERY_ITEMS.slice(); const s=GALLERY_SORT;
...
function initApp(){ fillSelect("crop_stage",STAGES); fillSelect("condition",CONDITION); fillSelect("purpose",PURPOSES); setNet(); updateCounts(); loadFarmers(); setTimeout(()=>syncNow(false),1500); }
(function boot(){
