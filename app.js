...
async function loadTeam(){
  ...
  if(!navigator.onLine){ const c=cacheGet("stats"); if(c){ renderTeam(c); el("kpi").insertAdjacentHTML("beforeend","<div class='muted' style='margin-top:6px'>Offline - showing last loaded.</div>"); } else el("kpi").innerHTML="<span class='muted'>Offline - connect to load the team dashboard.</span>"; return; }
  el("kpi").innerHTML="<span class='muted'>Loading...</span>";
  let s;
  try{ s=await apiRead({action:"stats"},3); cacheSet("stats",s); }
  catch(e){ s=cacheGet("stats"); if(!s){ el("kpi").innerHTML="<span class='muted'>Couldn't load - tap Refresh to retry.</span>"; return; } toast("Network slow - showing last loaded"); }
  renderTeam(s);
}
...
async function loadGallery(){
  ...
  if(GALLERY_SOURCE==="device"){
    ...
    if(!navigator.onLine){ const c=cacheGet("photos"); if(c&&c.length){ GALLERY_ITEMS=c; renderBands(); renderGallery(); } else g.innerHTML="<span class='muted'>Offline - connect to load team photos.</span>"; return; }
    g.innerHTML="<span class='muted'>Loading photos...</span>";
    try{ const out=await apiRead({action:"photos"},3); GALLERY_ITEMS=out.items||[]; cacheSet("photos",GALLERY_ITEMS); renderBands(); renderGallery(); }
    catch(e){ const c=cacheGet("photos"); if(c&&c.length){ GALLERY_ITEMS=c; renderBands(); renderGallery(); toast("Network slow - showing last loaded photos"); } else g.innerHTML="<span class='muted'>Couldn't load photos - tap Reload to retry.</span>"; }
  }
}
