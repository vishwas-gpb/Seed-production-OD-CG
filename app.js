...
function applyRole(role){
  ...
  el("tab-farmers").style.display=(viewer||reviewer)?"":"none"; el("tab-review").style.display=reviewer?"":"none";
...
function showView(v){ ["new","rec","review","farmers","photos","dash"].forEach(k=>{ const s=el("view-"+k); if(s) s.classList.toggle("hidden",k!==v); const t=el("tab-"+k); if(t) t.classList.toggle("active",k===v); }); if(v==="rec") render(); if(v==="dash") dashboard(); if(v==="photos") loadGallery(); if(v==="farmers"){ renderFarmerDir(); loadVisits(false); } if(v==="review"){ loadVisits(false).then(renderReviewList); } }
